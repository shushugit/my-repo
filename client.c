...
int main (int argc, char *argv[]) {
  ...
  connection_handler(cli_fd);//把自己傳入connection 
...
void connection_handler (int sockfd) {
    ...
    /**TODO 2:
	  ...
	  //server client要 read/write同步**/

    while (1) {
      int read = recv(sockfd, buf, sizeof(buf), 0);//read server傳送過來的訊息 
	  												//讀取的=接收到的 read=資料大小 
...
void file_download_handler(int sockfd, char filename[]) {
  ...
  if (fp) {
      while (read_sum < file_size) {
        memset(buf, '\0', MAX_SIZE);//初始buf 

        /**TODO 4:
          receive file data from server**/
          //send(sockfd, buf, write_byte, 0);//這裡是一次傳一個嗎?
          read_byte=recv(sockfd, buf,sizeof(buf),0);//接收server傳來的資料 

        /****/

        /* write file to local disk*/
        fwrite(&buf, sizeof(char), read_byte, fp);//把buf寫到fp 
        read_sum += read_byte;
      }
      fclose(fp);
